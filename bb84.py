...
def bob_measured_bits(bob_basis: list[str], alice_basis: list[str], alice_bits: list[str]) -> list[str]:
    """
...
def sift_keys(alice_basis: list[str],  bob_basis: list[str], alice_bits: list[str], bob_bits: list[str]):
    """
...
def sift_keys_helper(bit_list: list[str], alice_basis: list[str], bob_basis: list[str]):
    return [bit_list[i] for i in range(len(alice_basis)) if alice_basis[i] == bob_basis[i]]


def error_check(alice_bits: list[str], bob_bits: list[str]):
    """
...
def main():
    ...
    print(f"Alice Final: {alice_final} Bob Final: {bob_final} with error rate {error_rate}")
